...
async function run() {
  ...
  if (memberErr) throw memberErr  if (memberErr) throw memberErr
  ...
  const { data: inviteeProfile, error: profileErr } = await supabase.from('profiles').select('id').eq('email', inviteeEmail).single()
  if (profileErr || !inviteeProfile) throw new Error(`Failed to find profile for ${inviteeEmail}`)  const { error: splitErr } = await supabase
    .from('expense_splits')
    .insert([
